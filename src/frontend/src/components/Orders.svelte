<script>
  ...
  async function getTokenSymbol(principal) {
    const item = $canisters.find(
      (canister) => canister.canisterId === principal.toString()
    );
    if (item) {
      return item.symbol;
    }
          ...
          {#if showOrderForm}
            <tr>
              ...
              <td
                ><input
                  class="input-style"
                  bind:value={newOrder.fromAmount}
                  type="number"
                /></td
              >
              <td>
                ...
                {#if $auth.loggedIn}
                  <button class="btn-accept" title="Buy order">
                    {#if buyingOrder && $currentOrder.id === order.id}
                      <div class="loader buy-btn-loader" />
                    {:else}
                      <div
                        class="buy-btn-text"
                        on:click={() => buyOrder(order)}
                      >
                        <FontAwesomeIcon icon="check" />
                      </div>
                    {/if}
                  </button>
                  {#if authClient && order.owner.toText() === authClient
                        .getIdentity()
                        .getPrincipal()
                        .toText()}
                    <button
                    ...
                    >
                      {#if cancelingOrder && $currentOrder.id === order.id}
                        <div class="loader cancel-btn-loader" />
                      {:else}
